...
void LLossAVGLayer<Dtype>::LayerSetUp(
...
template <typename Dtype>
void LLossAVGLayer<Dtype>::Reshape(
  const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void LLossAVGLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  NOT_IMPLEMENTED;
}

template <typename Dtype>
void LLossAVGLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
  NOT_IMPLEMENTED;
}

#ifdef CPU_ONLY
STUB_GPU(LLossAVGLayer);
#endif

INSTANTIATE_CLASS(LLossAVGLayer);
REGISTER_LAYER_CLASS(LLossAVG);
