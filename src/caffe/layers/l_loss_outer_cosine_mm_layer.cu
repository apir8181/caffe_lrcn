...
__global__ void kernel_pairwise_error(
  const Dtype* S_frame, const Dtype* I_frame, Dtype margin_width,
  Dtype* out, const int count) {

   CUDA_KERNEL_LOOP(i, count) {
     Dtype pre_loss = margin_width - I_frame[i] * S_frame[i];
     out[i] = pre_loss > 0 ? 0.5 * I_frame[i] : 0;
   }
...
template <typename Dtype>
void LLossOuterCosineMMLayer<Dtype>::Forward_gpu(
  ...
  kernel_pairwise_error<Dtype><<<CAFFE_GET_BLOCKS(pair_size),
    CAFFE_CUDA_NUM_THREADS>>>(S_frame, I_frame, margin_width_,
                              pairwise_error, pair_size);
                              
  kernel_pairwise_loss<Dtype><<<CAFFE_GET_BLOCKS(pair_size),
    CAFFE_CUDA_NUM_THREADS>>>(S_frame, I_frame, margin_width_,
                              pairwise_loss, pair_size);
...
template <typename Dtype>
void LLossOuterCosineMMLayer<Dtype>::Backward_gpu(
...
INSTANTIATE_LAYER_GPU_FUNCS(LLossOuterCosineMMLayer);
} // namespace caffe
